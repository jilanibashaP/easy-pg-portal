...
const Stats = () => {
  ...
  const occupiedRooms = rooms.filter((room: any) => room.occupiedBeds > 0).length;
  const vacantRooms = totalRooms - occupiedRooms;
  const maintenanceRooms = rooms.filter((room: any) => room.occupiedBeds === 0 && room.totalBeds > 0).length;
  ...
  const generateMonthlyData = () => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      const monthPayments = payments.filter((p: any) => {
        ...
        return paidDate.getMonth() === month && paidDate.getFullYear() === currentYear;
      });
      const monthExpenses = expenses.filter((e: any) => {
        const expenseDate = new Date(e.date);
        return expenseDate.getMonth() === month && expenseDate.getFullYear() === currentYear;
      });
      const income = monthPayments.reduce((sum: number, p: any) => sum + (p.paidAmount || 0), 0);
      const expensesSum = monthExpenses.reduce((sum: number, e: any) => sum + (e.amount || 0), 0);
      monthlyData.push({
        name: monthName,
        revenue: income,
  ...
  const generateYearlyData = () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const income = yearPayments.reduce((sum: number, p: any) => sum + (p.paidAmount || 0), 0);
      const expensesSum = yearExpenses.reduce((sum: number, e: any) => sum + (e.amount || 0), 0);
      yearlyData.push({
        name: year.toString(),
        revenue: income,
        expenses: expensesSum,
